...
- **仅堆叠 me 节点（2026-07-08 调整）**：负载图只放本角色需处理的节点（媒介 13 项、市场 5 项），待他人处理节点（待他人审稿/待他人审核支出）不计入。节点集合与颜色见第 4 节。
...
负载图（TeamWorkloadChart）颜色规范：**仅堆叠需本角色处理的节点**（me 节点），待他人处理的节点不计入负载图。颜色按漏斗顺序由浅到深，待审稿统一红色高亮。
...
| 待确认合作 | `#a5b4fc` (indigo-300) | 合作意向确立 |
| 待执行(待提稿) | `#fcd34d` (amber-300) | 需要注意/卡点预警 |
| 待执行(待重新提稿) | `#fef08a` (yellow-200) | 返工警告 |
| **待审稿** | `#f87171` (red-400) | **核心阻塞点（红色高亮）** |
| 待执行(已定稿) | `#a78bfa` (violet-400) | 顺利推进/待收尾 |
| 待我审核支出 | `#818cf8` (indigo-400) | 财务前置节点 |
| 待申请支付 | `#60a5fa` (blue-400) | 最终财务节点 |

**市场负载（5 项，按堆叠顺序）**：

| 节点 | 颜色取值 (Tailwind) | 视觉意图 |
| --- | --- | --- |
| 待审批提报 | `#cbd5e1` (slate-300) | 初始/较轻量 |
| 待审批报价 | `#fcd34d` (amber-300) | 需要注意/卡点预警 |
| **待审稿** | `#f87171` (red-400) | **核心阻塞点（红色高亮）** |
| 待完结执行 | `#34d399` (emerald-400) | 顺利推进/待收尾 |
| 待我审核支出 | `#818cf8` (indigo-400) | 财务前置节点 |

> 注：流转链路（WorkflowPipeline）两角色均展示全部节点，仅 me/others 标注不同；负载图只堆叠本角色需处理（me）的节点。故媒介负载不含“待他人审稿/待他人审核支出”，市场负载不含“待他人审稿/待他人审核支出/待报价/待确认合作/待流量主确认合作”等。
